...
As mentioned, **error bars** are a helpful tool to show if different categorical variables are distinct from one another. You can edit the size of its size and color by specifying these within the geom_errorbar( ) function. Below is the same code chunk from the Bar Plot tutorial, with smaller error bar widths.
...
You can change the boldness of the error bars by instead using the "size=__" argument, or changing the color using "color=__". Try editing the chunk above to create your own plot!
